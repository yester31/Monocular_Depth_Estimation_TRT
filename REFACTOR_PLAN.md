...
결과를 **눈으로** 확인할 방법이 히트맵 한 장뿐이다. 이 저장소의 모델들은
깊이만 내놓지 않는데, 나머지 출력은 저장조차 되지 않는다.

### 11.1 지금 있는 것과 없는 것

| | 상태 |
| --- | --- |
| 깊이 히트맵 1장 | 각 `onnx2trt.py` 가 저장 |
| 포인트 클라우드 | `depth_anything_v2` / `depth_pro` / `moge_2` **3개만**, 각각 따로 작성됨 |
| 신뢰도·마스크·법선·카메라 자세 | **저장 안 됨** — 모델이 내는 정보의 절반 이상이 안 보인다 |
| 12개 결과 나란히 보기 | 없음. 폴더 12곳을 열어야 한다 |
| 테스트 이미지 | **`data/example.jpg` 한 장뿐** |

**히트맵의 문제**: 이미지마다 최소~최대를 0~255 로 정규화해 그린다. 그래서
**미터 단위 정보가 그림에서 사라지고**, 미터를 내는 모델과 상대 깊이만 내는
모델이 화면에서 똑같아 보인다. 비교 목적에는 오히려 오해를 부른다.

### 11.2 포인트 클라우드 — 모델을 3부류로 나눈다

일반화되는 것은 **역투영·색 입히기·마스킹·PLY 저장·뷰어**이고,
모델별 예외는 **"점 좌표를 어디서 얻는가"** 하나로 좁혀진다.

| 부류 | 모델 | 점을 얻는 법 |
| --- | --- | --- |
| **A. 점 좌표를 직접 출력** | `unidepth_v2`, `unik3d`, `moge_2`, `metric_anything` | 그대로 사용. 신뢰도/마스크로 걸러냄. moge_2·metric_anything 은 미터 스케일도 나옴 |
| **B. 깊이 + 카메라 정보** | `depth_pro` (`fov_deg` → 초점거리) | 역투영. **미터 단위로 정확** |
| **C. 깊이만** | `depth_anything_v2/ac/v3`, `distill_any_depth`, `vggt`, `streamvggt`, `metric3d_v2` | **초점거리를 가정해야 한다.** 형태는 맞고 절대 스케일은 임의 — 그 사실을 화면에 표시할 것 |

C 부류는 "보이긴 하지만 스케일은 믿을 수 없다"를 **뷰어가 말해야 한다.**
그러지 않으면 A/B 와 구분이 안 된다.

### 11.3 2D 보기 — 옵션으로 고를 수 있게

히트맵 외에 필요한 것들. 어느 것이 유용한지는 모델과 목적에 따라 다르므로
**하나로 정하지 않고 옵션으로 둔다.**

| 보기 | 답하는 질문 |
| --- | --- |
| 히트맵 + **실제 값 눈금(colorbar)** | "저 파란 곳이 몇 미터인가" — 지금은 알 수 없다 |
| **단면 그래프** (가로/세로 한 줄의 깊이 곡선) | "벽이 평평한가, 휘었는가" |
| **깊이 분포 히스토그램** | "이 모델이 값을 어느 구간에 몰아넣는가" |
| **지점 조회** (좌표 → 값) | "이 픽셀은 몇 미터인가" |
| **모델 간 차이 지도** | "두 모델이 어디서 갈리는가" |
| **12개 격자 비교** (깊이 스케일 통일) | 한 장면에 대한 전체 비교 |

### 11.4 선행 조건

**테스트 이미지를 늘리는 것이 먼저다.** 사진 한 장으로는 시각화를 만들어도
볼 것이 없고, 정확도 검증도 한 장에 기대고 있다. 실내/실외, 밝은/어두운,
가까운/먼 장면 5~10장.

### 11.5 순서

1. 테스트 이미지 확보
2. `core/pointcloud.py` — 역투영·색·마스크·PLY (부류 A/B/C 를 데이터로 선언)
3. `core/visualize.py` — 위 2D 보기들
4. 루트 `demo.py` — §3 Phase 4 가 요구했으나 미구현이던 그 CLI
5. 12개 격자 비교

**시각화는 최적화의 안전장치이기도 하다.** 정밀도나 빌드 설정을 바꾸면 결과가
미세하게 달라지는데, "이 정도 차이가 괜찮은가"는 숫자만으로 판단할 수 없다.
