...
| :--- | :--- |
| Input | RGB image (`434x560`) plus a text description |
| Output | Metric depth, base prediction, scale map A, and shift map B |
| Scale | Metric, in metres |
| TensorRT target | Published `da_s_vitl_vitl` stack, FP16 |

## Supported Encoders

| Depth encoder used here | Released upstream configuration |
| :--- | :--- |
| Depth Anything `vits` | Depth Anything ViT-S + DINOv2 ViT-L + CLIP ViT-L/14 |

Upstream publishes one TR2M checkpoint, `weights/da_s_vitl_vitl.pth`; no
alternative size is currently supported. The `vits` label describes the
Depth Anything branch only, not the complete three-encoder stack.
...
<!-- BENCH:BEGIN -->
- Input **434x560**, precision **fp16**, 100 iterations after 20 warmup
- **20.08 ms** mean, p50 20.08 / p90 20.16 / p99 20.21, min 19.96
- **49.80 fps**
