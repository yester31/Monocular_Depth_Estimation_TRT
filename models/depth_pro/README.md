...
| :--- | :--- |
| Input | RGB image, fixed `1536x1536` |
| Output | Canonical inverse depth and field of view |
| Scale | Metric after the model's focal-length conversion |
| TensorRT target | Fixed upstream model, FP16 |

## Supported Encoders

Depth Pro is distributed here as one fixed upstream configuration; there is
no encoder-size selector in these conversion scripts. Its `1536x1536` input
is also fixed rather than a `518x518` benchmark profile.

## TensorRT Performance

<!-- BENCH:BEGIN -->
- Input **1536x1536**, precision **fp16**, 100 iterations after 20 warmup
- **242.12 ms** mean, p50 242.17 / p90 243.15 / p99 243.65, min 240.37
- **4.13 fps**
- NVIDIA GeForce RTX 3080, TensorRT 10.16.1.11, torch 2.11.0+cu128, driver 591.86, GPU clock 1650 of 2130 MHz after the run
