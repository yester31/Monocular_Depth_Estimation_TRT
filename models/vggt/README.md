...
| :--- | :--- |
| Input | One RGB frame, `518x518` |
| Output | Depth; the split path can also return confidence and camera pose |
| Scale | Unknown global scale |
| TensorRT target | Fixed upstream model, FP16 |

## Supported Encoders

VGGT is exported as one fixed upstream configuration in this repository. The
choice described under [Two engine layouts](#two-engine-layouts) changes the
graph outputs and launch structure, not the encoder size.

## TensorRT Performance

<!-- BENCH:BEGIN -->
- Input **518x518**, precision **fp16**, 100 iterations after 20 warmup
- **52.58 ms** mean, p50 52.58 / p90 52.64 / p99 52.72, min 52.48
- **19.02 fps**
