...
| Output | Metric depth using the default Hypersim checkpoint |
| Scale | Metric |
...
| :--- | :--- |
| `vits` (Small) | `vits` (Small), `vitb` (Base), `vitl` (Large) |

The upstream architecture also contains a `vitg` configuration, but its
official checkpoint was still marked “Coming soon” when checked on
2026-08-14. The commands below download relative and metric checkpoints for
the three released sizes; this repository benchmarks `vits`.

## TensorRT Performance

<!-- BENCH:BEGIN -->
- Input **518x518**, precision **fp16**, 100 iterations after 20 warmup
- **4.31 ms** mean, p50 4.31 / p90 4.35 / p99 4.42, min 4.24
- **232.11 fps**
