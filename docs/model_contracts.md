...
#### 결론 — 이 두 모델은 정적 엔진과 근본적으로 맞지 않는다

`depth_anything_ac` 는 native 로 업스트림과 **비트 단위 일치**했다.
`unidepth_v2`/`unik3d` 는 어떤 고정 크기로도 일치시킬 수 없다.

| 선택 | 오차 | 성격 |
| --- | ---: | --- |
| bench 518×518 | 215% | 다른 모델과 속도 비교 가능 |
| native 896×672 | 17.8% | 최선의 근사. 픽셀 2.24× 라 속도 비교 불가 |
| 원본 투입 | 0% | **정적 엔진 불가** (이미지마다 크기가 달라짐) |

**metric depth 값을 신뢰해야 하는 용도라면 이 두 모델은 TensorRT 정적 엔진으로
쓰기 어렵다.** 비교표에 이 한계를 반드시 명시해야 한다 —
"518×518 에서 잰 unik3d 의 metric depth 는 업스트림의 3.15배" 라는 사실을
모르면 수치를 오해한다.
