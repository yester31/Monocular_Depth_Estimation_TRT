...
| **D11** | **결함 아님 — 의도된 트레이드오프.<br>한계를 명시하는 것으로 종결** | 기존 코드가 이미 모델의 리사이즈 규칙을 주석으로 파악해 두고<br>정적 엔진을 위해 518×518 stretch 를 택한 것이었다.<br>측정되지 않았던 대가(metric 3.15배)를 소스와 문서에 명시.<br>**코드 동작은 변경 없음** |
...
| --- | --- |
| 코드 | **변경 없음.** 518×518 유지 |
| `onnx2trt.py` | 상단에 WARNING — metric depth 가 업스트림의 3.15배, 측정치 표 포함 |
...
실측이 이 해석을 뒷받침한다(§5.7). keep-ratio 리사이즈 팩터가 `0.2716 → 1.1892`
로 4.4배 달라져도 깊이는 스케일 1.00~1.12 안에 머문다. 출력이 입력 크기에
불변이라는 건 곧 **아직 실제 카메라에 묶이지 않았다**는 뜻이다.
...
| `unik3d`/`unidepth_v2` | **문제 있음.** 스케일 최대 3.15배 |

`metric3d_v2` 의 측정은 다른 것도 보여준다. keep-ratio 리사이즈 팩터가
`0.2716 → 1.1892` 로 **4.4배** 달라져도 깊이는 거의 그대로다. 그게 바로
...
`Cast → Div → (Sub) → (Div) → Transpose` 를 붙이고, `ab_input_dtype.py` 가
두 엔진을 같은 데이터로 비교한다.
...
`h2d` 는 두 모델 모두 전송량 감소와 정확히 일치한다(1/4 로 줄었다).
**갈리는 곳은 compute 다.**

- `depth_anything_v2` — compute 가 오히려 미세하게 줄었다.
  preamble 이 흡수됐다. h2d 절감이 그대로 총시간 절감이 된다.
- `depth_pro` — compute 가 **+2.99 ms 늘어** h2d 절감 6.77 ms 의 절반가량을
  먹었다. 1536² 에서는 preamble 자체가 공짜가 아니다.

### 총시간 −1.57% 는 "이득"이라 부를 수 없다

같은 엔진을 다른 시점에 다시 재면 **약 2% 움직인다**(depth_anything_v2
4.23 → 4.31 ms, 한 런 안의 stdev 는 0.12 ms). depth_pro 의 −1.57% 는 그
폭 안에 있다. 반면 `h2d` 와 `compute` 는 **같은 프로세스에서 연속으로** 잰
값이라 이 흔들림을 공유하지 않는다 — 그래서 구간 분리가 판정 근거다.
...
**preamble 은 엔진 정밀도로 실행된다.** 즉 fp16 엔진에서는 정규화가 fp16 로
수행되고, 호스트에서 float32 로 하던 것과 마지막 자리가 다르다. 이 차이는
이미 기록된 엔진↔ONNX 오차(0.03~0.92%)와 같은 대역이므로 별도 결함이 아니지만,
...
| `depth_pro` | **보류** — h2d 절감은 확실하나 compute 증가가 상쇄. 총시간 이득이 재현되는지 확인 필요 |
