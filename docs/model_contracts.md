...
| **D13** | **확인 필요 (실측 대기)** | `metric_anything` 은 518×518 로 stretch 한 뒤 `aspect_ratio=1.0` 으로 intrinsics 를 만든다.<br>같은 후처리를 쓰는 `moge_2` 는 388×518 로 종횡비를 지킨다 |
...
아니라는 것이다. 점군을 만드는 `demo_pointcloud.py` 는 그 왜곡을 그대로 물려받는다.

`metric_anything/onnx2trt.py` 의 `resize_image()` 에는 `resize_mode=2`
("keep aspect ratio") 가 이미 구현돼 있고 쓰이지 않는다 — 작성자도 알고 있었다는 뜻이다.

D11 과 달리 이건 **고정 크기로 고칠 수 있다.** `moge_2` 처럼 4:3 기준
아스펙트 보존 크기를 쓰면 된다. 다만 그러면 `moge_2` 와 마찬가지로 4:3 에
묶이므로, 결정 전에 실측이 필요하다.

**상태: 코드 판독까지만 끝났다. 실측 전에는 결함으로 확정하지 않는다** —
D4·D5·D10 이 정확히 이 단계에서 오진이었다.
