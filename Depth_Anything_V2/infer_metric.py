...
from metric_depth.depth_anything_v2.util.transform import Resize, NormalizeImage, PrepareForNet
...
def infer_one_vis(model, image_path, encoder, dataset):
    # Load image.
    ...
    # visualization
    depth = 255.0 - (depth0 - depth0.min()) / (depth0.max() - depth0.min()) * 255.0
    ...
    cv2.imwrite(os.path.join(outdir, os.path.splitext(os.path.basename(image_path))[0] + f'_{encoder}_{dataset}_Torch.png'), depth)

    save_npz(depth0, os.path.splitext(os.path.basename(image_path))[0] + f'_{encoder}_{dataset}')
...
def main():
    ...
    infer_one_vis(model, image_path, encoder, dataset)
